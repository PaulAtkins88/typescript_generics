...
This repo shows how generics remove duplication across entities (e.g., `User`, `Order`) by sharing type-safe base interfaces and classes.

## Key Concepts

- Generic interfaces: `IRepository<T>`, `IService<T>`, `IResponse<T>`
- Generic base classes: `BaseService<T>`, `BaseController<T>`
...
src/
  ...
  interface/
    ...
    IService.ts              # business logic abstraction
  model/
    Order.ts
    User.ts
  repository/
...
class OrderService
OrderService --|> BaseService~Order~
...
UserController --|> BaseController~User~

class OrderController
OrderController --|> BaseController~Order~
```
...
UserController->>UserService: getById(id)
UserService->>UserRepository: getById(id)
UserRepository->>DataSource: Read user by id
DataSource-->>UserRepository: User
UserRepository-->>UserService: IResponse<User>
UserService-->>UserController: IResponse<User>
...
- `IRepository<T>` and `IService<T>` may look identical in basic CRUD examples. This is intentional for teaching:
  - Repositories: “how” data is accessed (DB, file, API, memory)
  - Services: “what” business rules and orchestration are applied
  - As complexity grows, services diverge (validation, cross-repo flows, DTO transforms).

- Generic base classes prevent duplication:
  - `BaseService<T>` implements CRUD once, services inherit it.
  - `BaseController<T>` standardizes HTTP flow and error handling.
...
1. Create model type: `src/model/Product.ts`
2. Implement repository: `InMemoryProduct.repository.ts` (implements `IRepository<Product>`)  
   - or a DB-backed repository
3. Create service: `Product.service.ts` extending `BaseService<Product>`  
   - constructor accepts `IRepository<Product>`
4. Create controller: `Product.controller.ts` extending `BaseController<Product>`
5. Add routes: `Product.route.ts` wiring Repository → Service → Controller
